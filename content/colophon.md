...
[Pillow](https://python-pillow.org) and the same brand fonts as the site. The layout is
intentionally simple: the post title in Cormorant at the largest size that fits in three
lines, a teal rule, an optional description in Nunito, and a `joe.dev · Joe Beda`
wordmark at bottom right — all on the same warm parchment background as the site.
